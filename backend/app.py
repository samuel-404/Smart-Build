...
def get_data() -> Dict[str, Any]:
    """Get component data from Supabase, with basic memory caching and local JSON fallback"""
    global _DATA_CACHE
    ...
    # Load local seed data as fallback if Supabase returns nothing/is not configured
    seed_data = {}
    if not components_raw or not usage_profiles_raw or not budget_tiers_raw:
        try:
            base_dir = os.path.dirname(os.path.abspath(__file__))
            seed_path = os.path.join(base_dir, "data", "seed_data.json")
            if os.path.exists(seed_path):
                logger.info(f"Loading local fallback seed data from {seed_path}")
                with open(seed_path, "r", encoding="utf-8") as f:
                    seed_data = json.load(f)
            else:
                logger.warning(f"Local seed data file not found at {seed_path}")
        except Exception as e:
            logger.error(f"Error loading local seed data: {e}")

    if not components_raw and "components" in seed_data:
        logger.info("Using local fallback components data")
        structured_components = seed_data["components"]
    else:
    ...
    if not usage_profiles_raw and "usage_profiles" in seed_data:
        logger.info("Using local fallback usage profiles")
        usage_profiles = {}
        for k, v in seed_data["usage_profiles"].items():
            profile_copy = dict(v)
            profile_copy["id"] = k
            usage_profiles[k] = profile_copy
    else:
        usage_profiles = {p["id"]: p for p in usage_profiles_raw}
        
    if not budget_tiers_raw and "budget_tiers" in seed_data:
        logger.info("Using local fallback budget tiers")
        budget_tiers = {}
        for k, v in seed_data["budget_tiers"].items():
            tier_copy = dict(v)
            tier_copy["id"] = k
            budget_tiers[k] = tier_copy
    else:
